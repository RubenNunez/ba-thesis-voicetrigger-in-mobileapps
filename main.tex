...
Die Forschung im Bereich der Spracherkennung ist ein aktives Forschungsgebiet. Die Jahre 2010
...
Amazon bekannt. Apple veröffentlichte seinen Sprachassistenten Siri im Jahr 2011 mit dem Release 
von iOS 5. 

\subsection{Zeitliche Entwicklung der Spracherkennung} 
Das Jahrzehnt von 2010 bis 2020 erlebte bedeutende Fortschritte in der automatischen 
Spracherkennung. Viele Menschen nutzen nun täglich Spracherkennung, beispielsweise für 
Sprachsuchanfragen oder Interaktionen mit Sprachassistenten wie Amazon Alexa und Siri von Apple. 
Vor 2010 war die Nutzung der Spracherkennung selten. Angesichts der bemerkenswerten Veränderungen 
im Bereich der Spracherkennung im letzten Jahrzehnt stellt sich die Frage, was das nächste 
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[scale=1.5, every node/.style={scale=0.8}] 
        ...
        % Beschriftung
        \node[align=center, below] at (0,-0.5) {2010};
        \node[align=center, below] at (10,-0.5) {2020};

        % Ereignisse
		\node[align=center, above] (hybrid) at (2,0.5) {Hybrid};
		\node[align=center, above] (libri) at (4,0.2) {Deep LibriSpeech\\Google};
		\node[align=center, above] (human) at (6,0.5) {Human-level\\Switchboard};
		\node[align=center, above] (kaldi) at (8,0.2) {Kaldi HMM/DNN\\Speech};
		\node[align=center, below] (google) at (3,-0.5) {Google streaming\\on-device transducer};
		\node[align=center, below] (siri) at (5,-1) {Apple Siri\\Amazon Echo};
		\node[align=center, below] (speech2) at (7,-0.5) {Deep Speech 2};
		\node[align=center, below] (encoder) at (9,-1) {Encoder-decoder models\\for ASR};

		% Linien zu den Ereignissen
		\draw (hybrid) -- (2,0);
		\draw (libri) -- (4,0);
		\draw (human) -- (6,0);
		\draw (kaldi) -- (8,0);
		...
		\draw (encoder) -- (9,0);
    \end{tikzpicture}
    \caption{Timeline der Spracherkennung}
	\label{fig:timeline} % für spätere Referenzen
\end{figure}

\noindent \newline
Deep Learning hat die Spracherkennung revolutioniert, insbesondere durch die Sammlung grosser 
transkribierter Datensätze und Fortschritte in Grafikprozessoreinheiten. Interessanterweise hat 
Richard Hamming, ein Mathematiker, Informatiker und Elektroingenieur, viele Vorhersagen über die 
Zukunft der Computertechnik gemacht, von denen viele eingetreten sind. Er betonte die Wichtigkeit 
der Praxis bei der Vorhersage der Zukunft und machte seine Vorhersagen öffentlich. 
(\cite{hannun2021history})
