...
Leistung dienen verschiedene Metriken, die in diesem Kapitel vorgestellt werden. Neben der bereits 
...
Forschung. Die Wahl von PyTorch ist somit eine gute Wahl.
...
mobile App. Es ist zum einen zu gross und zum anderen zu energieintensiv in der Inferenz. 
...
wav2vec2 als Teacher-Model und ein kleineres Modell als Student-Model. Doch an dieser Stelle 
...
Aufgabenstellung ging schon hervor, dass ein eigenes Deep Learning Modell für die Triggerwort 
...
wurde schlussendlich der Ansatz von (\cite{barazanji2023heyditto}) verwendet. Die Architektur 
besteht mehrheitlich aus Convolutional Layers und einem Long-Short-Term-Memory LSTM Layer. Der 
Vorteil dieser Architektur ist, dass sie sehr energie effizient ist, wie auch sehr schnell in 
der Inferenz. Mehr dazu im Kapitel \ref{sec:evaluation}. Was besonders Markant ist, ist die 
Verwendung von Convolutional Layers in Kombination mit einem LSTM Layer, welcher die zeitliche 
...
Wie bereits in den Grundlagen beschrieben, werden die Sprachdaten in ein Spektrogramm transformiert.
Diese Transformation ist essentiell der Modelarchitektur. Für die Transformation wurde ein Pytorch 
Modul entwickelt, welches die Transformation der Sprachdaten übernimmt. Kurz zum Konzept von PyTorch 
Module. PyTorch Module erben von der Klasse nn.Module und sind die Grundbausteine von PyTorch. Sie 
haben einen Vorteil welcher vorallem bei der Integration in eine mobile App wichtig ist. Aber dazu 
später mehr. Das PyTorch Modul für die Transformation der Sprachdaten ist in der Abbildung 
\ref{fig:transform} dargestellt. 
...
\newpage \subsubsection{Training des Modells}
Hier werden die Resultate des Trainings beschrieben. Der Datensatz wurde in Trainingsdaten und 
Validierungsdaten aufgeteilt. Die Trainingsdaten wurden für das Training des Modells verwendet. 
Die Validierungsdaten wurden verwendet, um das Modell während des Trainings zu validieren. Neben 
der Aufteilung der Daten in Trainings- und Validierungsdaten wurde auch ein finaler Testdatensatz
erstellt. Dieser wurde verwendet, um das Modell nach dem Training zu evaluieren und eine Aussage 
über die Leistungsfähigkeit des Modells zu treffen.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/train/batch_accuracy.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/train/batch_f1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/train/batch_loss.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/train/epoch_accuracy.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/train/epoch_f1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{img/train/epoch_loss.png}
    \caption{Loss pro Epoche}
    \label{fig:epoch_loss}
\end{figure}

\noindent \newline
Diese Diagramme bieten eine detaillierte Übersicht über die Leistung des Modells während des 
Trainingsprozesses. Es zeigt das Das Model sich den Daten anpasst und die Metriken sich verbessern 
über die Zeit. 

\newpage

\subsubsection{Real Time Anwendung}
