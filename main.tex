...
Da wir nun die Grundlagen der Fourier-Analyse kennen, können wir uns mit dem Spektrogramm
beschäftigen. Ein Spektrogramm ist eine visuelle Darstellung der Frequenzkomponenten eines
Signals über der Zeit. Definiert ein Spektrogramm (\cite[Chapter~15.2.1]{tarr2018hackaudio})
folgendermassen:

\begin{displayquote}
	''A spectrogram is a three-dimensional visualization of a signal’s amplitude over frequency and
	time. Many audio signals are comprised of multiple frequencies happening at the same time, and
	are also signals that change to different frequencies at different times. Therefore, the
	spectrogram is a common way for audio engineers to analyze a complex signal.''
\end{displayquote}

\noindent
Im Kontext von Maschinellem Lernen und insbesondere in der Spracherkennung spielt das Spektrogramm 
eine entscheidende Rolle. Wenn wir ein Audiosignal in seine Frequenzkomponenten zerlegen, verlieren 
wir durch die Anwendung der FFT die zeitliche Information des Signals. In Anwendungen wie der 
Spracherkennung ist es jedoch unerlässlich, nicht nur zu wissen, welche Frequenzen in einem Signal 
vorhanden sind, sondern auch wann sie auftreten. Ein Spektrogramm löst dieses Problem, indem es 
Frequenzen in Beziehung zur Zeit darstellt. Dies ist besonders wichtig, um beispielsweise die 
Reihenfolge von gesprochenen Wörtern in einem Satz zu erkennen \cite{chaudhary2020}. Ein 
Spektrogramm bietet somit eine Kombination aus zeitlichen und frequenziellen Informationen, was es 
zu einem mächtigen Werkzeug für die Spracherkennung und andere Anwendungen im Bereich Maschinelles 
Lernen macht.
